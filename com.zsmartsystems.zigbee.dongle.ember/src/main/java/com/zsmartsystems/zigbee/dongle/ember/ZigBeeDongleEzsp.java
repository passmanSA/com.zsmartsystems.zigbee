...
public class ZigBeeDongleEzsp implements ZigBeeTransportTransmit, ZigBeeTransportFirmwareUpdate, EzspFrameHandler {
    ...
    //FIXME ces constantes sont définies ailleurs, voir ce qui est le mieux
    /**
    ...
    //FIXME ajout de MMIckael, à garder?
    /**
     ...
     * called.
     */
    //FIXME voir si compatible avec la façon dont Mickael gère le npc
    ...
    public void sendCommand(final int msgTag, final ZigBeeApsFrame apsFrame) {
        ...
        if (apsFrame.getAckRequest()) {
            //FIXME on garde ce if?? la ligne qu'il contenait est juste en dessoous
        }
        ...
        executorService.execute(new Runnable() {
            @Override
            public void run() {
                //FIXME grosse modif, voir si compatible avec ce qu'a fait Mickael
                synchronized (isConfiguredSync) {
                    if (!isConfigured) {
                        logger.debug("EZSP Dongle is not configured. Frame not sent: {}", apsFrame);
                        return;
...
//        if (ezspVersion.getProtocolVersion() < 8) {
    ...
    private boolean initialiseEzspProtocol() {
        ...
        //FIXME bien valider qu'on garde cette gestion de la version
